...
const createThread = (title = '新的聊天'): ChatThread => {
...
export default function InteractionDesignView({ variant = 'page', floating = false, onToggleFloating, onClose }: InteractionDesignViewProps) {
  ...
  const [threads, setThreads] = useState<ChatThread[]>(() => [createThread('创作助手')]);
  ...
  const sendMessage = async () => {
    ...
    const contextText = references.length ? `\n\n附件上下文：${references.map((file) => file.name).join('、')}` : '';
    ...
    updateActiveThread((thread) => ({
      ...thread,
      title: thread.messages.length ? thread.title : createTitle(content),
      updatedAt: now,
    ...
    try {
      ...
      const assistantMessage: OpenFMVChatMessage = {
        role: 'assistant',
        content: response.ok ? response.content : response.error || 'AI 没有返回可用回复。',
      };
      updateActiveThread((thread) => ({
        ...thread,
      ...
      updateActiveThread((thread) => ({
        ...thread,
        updatedAt: new Date().toISOString(),
        messages: [...thread.messages, { role: 'assistant', content: '聊天失败，请检查设置中的 CLI 是否可用。' }],
      }));
    } finally {
      setIsSending(false);
    }
  ...
  const historyList = (
    <aside className={isPanel ? 'absolute right-12 top-12 z-30 max-h-[420px] w-[280px] overflow-hidden rounded-[14px] border border-white/10 bg-[#181818] shadow-[0_18px_70px_rgba(0,0,0,0.45)]' : 'flex w-[300px] shrink-0 flex-col border-r border-white/8 bg-[#202020]'}>
      <div className="flex h-12 items-center justify-between border-b border-white/8 px-4">
        <div className="text-sm font-semibold text-white">历史会话</div>
        <button type="button" onClick={startThread} className="grid h-8 w-8 place-items-center rounded-[10px] text-openfmv-sub transition hover:bg-white/[0.08] hover:text-white" title="新建聊天">
          <Plus size={18} />
        </button>
      </div>
      <div className={isPanel ? 'max-h-[368px] overflow-y-auto p-2' : 'min-h-0 flex-1 overflow-y-auto p-3'}>
        {sortedThreads.map((thread) => {
          ...
          const lastMessage = thread.messages.at(-1)?.content || '还没有消息';
          return (
            <button key={thread.id} type="button" onClick={() => selectThread(thread.id)} className={`mb-2 block w-full rounded-[12px] border p-3 text-left transition ${isActive ? 'border-orange-300/35 bg-orange-400/[0.10]' : 'border-white/8 bg-white/[0.035] hover:bg-white/[0.06]'}`}>
              <div className="flex items-center justify-between gap-3">
                ...
                <span className="shrink-0 text-xs text-openfmv-muted">{formatTime(thread.updatedAt)}</span>
              </div>
        ...
        <div className="border-t border-white/8 p-3">
          <div className="flex items-center gap-3 rounded-[12px] border border-white/10 bg-white/[0.045] p-3">
            ...
            <span className="min-w-0">
              <span className="block truncate text-sm font-semibold text-white">{selectedAgent?.name || '本地 AI'}</span>
              <span className="mt-0.5 block truncate text-xs text-openfmv-muted">{selectedModel || '未选择模型'}</span>
            </span>
          </div>
        </div>
      )}
        ...
        <header data-chat-drag-handle={isPanel ? true : undefined} className={`flex h-14 shrink-0 items-center justify-between ${isPanel ? 'cursor-move px-4' : 'border-b border-white/8 px-6'}`}>
          {isPanel ? (
            <div className="flex flex-1 justify-end gap-1">
              <button type="button" onClick={() => setShowHistory((current) => !current)} className={`grid h-8 w-8 place-items-center rounded-[9px] transition hover:bg-white/[0.08] hover:text-white ${showHistory ? 'bg-white/[0.08] text-white' : 'text-white/65'}`} title="历史会话">
                <History size={18} />
              </button>
              <button type="button" onClick={onToggleFloating} className={`grid h-8 w-8 place-items-center rounded-[9px] transition hover:bg-white/[0.08] hover:text-white ${floating ? 'bg-white/[0.08] text-white' : 'text-white/65'}`} title={floating ? '停靠窗口' : '独立窗口'}>
                <PanelTop size={18} />
              </button>
              <button type="button" onClick={onClose} className="grid h-8 w-8 place-items-center rounded-[9px] text-white/65 transition hover:bg-white/[0.08] hover:text-white" title="关闭">
                <X size={18} />
              </button>
            </div>
          ) : (
            <div className="min-w-0">
              ...
              <div className="mt-0.5 truncate text-xs text-openfmv-muted">正在和 {selectedAgent?.name || '本地 AI'} · {selectedModel || '默认模型'} 聊天</div>
            </div>
          )}
        </header>
          ...
          {activeThread.messages.length ? (
            ...
            <div className={isPanel ? 'flex h-full items-end pb-40' : 'grid h-full place-items-center'}>
              <div className={isPanel ? 'w-full text-left' : 'text-center'}>
                ...
                {isPanel ? (
                  <>
                    <div className="text-[28px] font-bold leading-tight text-[#b8d3ff]">你好，</div>
                    <div className="mt-1 text-[24px] font-bold leading-tight text-white/82">今天需要我做些什么？</div>
                    <div className="mt-8 flex flex-col items-start gap-3">
                      {['帮我理清思路，解决问题', '生成一个故事分支', '你能做些什么？'].map((item) => (
                  ...
                  <>
                    <div className="mt-4 text-lg font-semibold text-white">开始一段聊天</div>
                    <div className="mt-2 text-sm text-openfmv-muted">输入消息后会使用设置中选择的 AI 和模型。</div>
                  </>
                )}
              </div>
            </div>
        ...
        <div className={`shrink-0 ${isPanel ? 'bg-[#111] p-3' : 'border-t border-white/8 bg-[#1d1d1d] p-4'}`}>
          <div className={isPanel ? 'w-full' : 'mx-auto max-w-4xl'}>
              ...
              <div className="mb-3 flex flex-wrap gap-2">
                {references.map((file) => (
                  <span key={file.id} className="inline-flex max-w-[260px] items-center gap-2 rounded-full border border-white/10 bg-white/[0.06] px-3 py-1.5 text-xs text-openfmv-sub">
                    ...
                    <button type="button" onClick={() => setReferences((current) => current.filter((item) => item.id !== file.id))} className="text-openfmv-muted transition hover:text-white" title="移除附件">
                      <X size={12} />
                    </button>
                  </span>
                ))}
            ...
            <div className="flex items-end gap-3 rounded-[16px] border border-white/10 bg-white/[0.055] p-3">
              <button type="button" onClick={() => fileInputRef.current?.click()} className="grid h-10 w-10 shrink-0 place-items-center rounded-[12px] text-openfmv-sub transition hover:bg-white/[0.08] hover:text-white" title="添加附件">
                <Paperclip size={18} />
              </button>
